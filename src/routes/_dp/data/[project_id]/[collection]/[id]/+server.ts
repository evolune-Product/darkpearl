 ...
 *
 * GET    /_dp/data/{project_id}/{collection}/{id} - Get record
 * PATCH  /_dp/data/{project_id}/{collection}/{id} - Update record
 * DELETE /_dp/data/{project_id}/{collection}/{id} - Delete record
 *
 * Security:
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		await pb.collection('_dp_projects').update(project_id, { data })
...
async function upload_files_to_assets(
	...
	const project = await pb.collection('_dp_projects').getOne(project_id)
	const before_count = (project.assets as string[] || []).length

	const updated = await pb.collection('_dp_projects').update(project_id, upload_form)
