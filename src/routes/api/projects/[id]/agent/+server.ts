...
import { getProject, updateProject, createSnapshot, validateUserToken, unauthorizedResponse } from '$lib/server/pb'
import { type LLMProvider, type AgentMessage } from '$lib/ai/sdk-agent'
import { calculateCost } from '$lib/ai/types'

// LLM configuration from environment variables (platform-provided)
const LLM_PROVIDER = (env.LLM_PROVIDER as LLMProvider) || 'anthropic'
...
// Track running agents to prevent duplicate runs (with timestamp for timeout)
const runningAgents = new Map<string, number>()
const AGENT_TIMEOUT = 5 * 60 * 1000 // 5 minutes timeout

// Check if agent is stuck (running for too long)
function isAgentStuck(projectId: string): boolean {
	const startTime = runningAgents.get(projectId)
	if (!startTime) return false
	return Date.now() - startTime > AGENT_TIMEOUT
}

// Clear stuck agent
function clearStuckAgent(projectId: string): boolean {
	if (isAgentStuck(projectId)) {
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	// Force clear the running agent
	const wasRunning = runningAgents.has(projectId)
	runningAgents.delete(projectId)

	// Also reset agent_status in database
	try {
		await updateProject(projectId, { agent_status: 'idle' })
		return json({
			success: true,
			cleared: wasRunning,
			message: wasRunning ? 'Agent cleared successfully' : 'No running agent found'
		})
	} catch (error: any) {
		if (error.status === 404) {
			return json({ error: 'Project not found' }, { status: 404 })
		}
		return json({ error: 'Failed to clear agent' }, { status: 500 })
	}
}
...
export const POST: RequestHandler = async ({ params, request, getClientAddress }) => {
	...
	try {
		...
		if (runningAgents.has(projectId)) {
			// Check if it's stuck and auto-clear
			if (clearStuckAgent(projectId)) {
				console.log(`[Agent] Auto-cleared stuck agent for project ${projectId}`)
			} else {
		...
		if (!llmConfig.apiKey) {
			return json({ error: 'AI service is not available. Please contact support.' }, { status: 503 })
		}
		...
		// Mark agent as running with timestamp
		runningAgents.set(projectId, Date.now())
