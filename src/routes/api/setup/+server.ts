...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		// 5. Keep users collection open for public registration (B2C model)
		console.log("[Setup] Ensuring users collection allows registration...")
		try {
			const usersCollection = await pb.collections.getOne("users")
			await pb.collections.update(usersCollection.id, {
				createRule: "" // empty string = anyone can register
			})
			console.log("[Setup] Users collection updated - registration enabled")
		} catch (err: any) {
			console.error("[Setup] Failed to update users collection rules:", err.message)
			// Non-fatal, continue
		}

		// 6. Save auth credentials for server-side operations
