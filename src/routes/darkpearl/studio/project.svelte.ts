...
export class ProjectStore {
    ...
    async subscribe() {
        try {
            this.unsubscribe_fn = await pb.collection('_dp_projects')
                .subscribe(this.project_id, (e) => {
                    if (e.action === 'update') {
                        const incoming = e.record as unknown as Project;

                        // Check what changed - compare all fields for selective updates
                        ...
                        const incoming_code = incoming.frontend_code || '';
                        const current_code = this.project?.frontend_code || '';
                        const code_changed = incoming_code !== current_code;
                        ...
                        if (!chat_changed && !data_changed && !code_changed && !content_changed && !design_changed) {
                            return;
                        }
                        ...
                        // Always sync data changes (for manual CRUD operations)
                        if (data_changed) {
                            updates.data = incoming.data;
                        }

                        // Sync agent_chat when it changes
                        if (chat_changed) {
                            updates.agent_chat = incoming.agent_chat;
                        }

                        // Sync content/design when they change (or when agent finishes)
                        if (content_changed || agent_just_finished) {
                            updates.content = incoming.content;
                        }
                        if (design_changed || agent_just_finished) {
                            updates.design = incoming.design;
                        }

                        // CRITICAL: Sync frontend_code whenever it changes
                        // This ensures preview updates immediately when write_code tool runs
                        // (Preview has its own debouncing and freeze detection)
                        if (code_changed || agent_just_finished) {
                            updates.frontend_code = incoming.frontend_code;
                        }
    ...
    async update(data: Partial<Project>) {
        ...
        // Persist to server
        await pb.collection('_dp_projects').update(this.project_id, data);
    }
}
