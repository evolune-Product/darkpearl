<script lang="ts">
  ...
  import { HelpCircle, ChevronRight, ChevronDown, LogOut, Github, Upload, Download, Link, Unlink } from "lucide-svelte";
  ...
  type HeaderProps = {
    ...
    github_repo?: { full_name: string; owner: string; name: string } | null;
  ...
  let dashboard_href = $derived(kit_id ? `/darkpearl/dashboard?kit=${kit_id}` : "/darkpearl/dashboard");
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content align="end" class="w-64">
          ...
          <div class="px-2 py-1.5">
            <span class="text-xs text-[var(--builder-text-secondary)] flex items-center gap-1.5">
              <Github size={12} />
              GitHub
            </span>
          </div>
          {#if github_repo}
            <div class="px-2 py-1 text-xs text-[var(--builder-text-secondary)]">
              Linked: <span class="text-[var(--builder-accent)]">{github_repo.full_name}</span>
            </div>
            <DropdownMenu.Item
              onclick={() => on_github_push?.()}
            ...
            <DropdownMenu.Item
              onclick={() => on_github_unlink?.()}
              class="cursor-pointer hover:bg-[var(--builder-bg-secondary)] flex items-center gap-2 text-red-400"
            >
              <Unlink size={14} />
              Unlink Repo
            </DropdownMenu.Item>
...
<Dialog.Root bind:open={show_help_dialog}>
  <Dialog.Content class="help-dialog">
    <Dialog.Header>
      <Dialog.Title>darkpearl Help</Dialog.Title>
      <Dialog.Description>
        Learn how to use the different features of darkpearl.
