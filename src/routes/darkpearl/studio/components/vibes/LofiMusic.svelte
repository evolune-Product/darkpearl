<script lang="ts">
	...
	onMount(() => {
		const stored_mute = localStorage.getItem("darkpearl-studio-mute")
		if (stored_mute !== null) {
			is_muted = stored_mute === "true"
		}
		const stored_stream = localStorage.getItem("darkpearl-lofi-stream")
	...
	function toggle_mute() {
		is_muted = !is_muted
		localStorage.setItem("darkpearl-studio-mute", String(is_muted))
	}

	function next_stream() {
		current_index = (current_index + 1) % streams.length
		localStorage.setItem("darkpearl-lofi-stream", String(current_index))
	}
</script>
