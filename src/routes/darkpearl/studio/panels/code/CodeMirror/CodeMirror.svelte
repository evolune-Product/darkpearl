...
<script lang="ts">
  ...
  let darkpearl_completions_compartment = new Compartment();
  ...
  onMount(async () => {
    ...
    const themedEditor = themeModule.create_themed_editor();
    const darkpearlCompletions = extModule.create_darkpearl_completions(collections, content_fields);
    ...
    const initialState = EditorState.create({
      ...
      extensions: [
        ...
        darkpearl_completions_compartment.of(darkpearlCompletions),
        keymap.of([
          ...standardKeymap,
          indentWithTab,
          {
  ...
  watch(
    ...
    () => {
      if (editorView && darkpearl_completions_compartment && extModule) {
        const newCompletions = extModule.create_darkpearl_completions(collections, content_fields);
        editorView.dispatch({
          effects: darkpearl_completions_compartment.reconfigure(newCompletions),
        });
      }
    },
  );
