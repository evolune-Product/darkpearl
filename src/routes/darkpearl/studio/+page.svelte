<script lang="ts">
  ...
  import GitHubLinkDialog from "./components/GitHubLinkDialog.svelte";
    ...
    if (!data.project_id && data.domain && !project_id) {
      ...
      pb.collection("_dp_projects")
        ...
        .catch(() => {
          ...
          goto(`/darkpearl/new?domain=${encodeURIComponent(data.domain)}`);
        });
    }
  });
  ...
  const PREVIEW_POSITION_KEY = "darkpearl:preview_position";
  ...
  let show_github_link_dialog = $state(false);
  ...
  onMount(() => {
    ...
    const check_initial_prompt = () => {
      const storage_key = `darkpearl:initial_prompt:${project_id}`;
  ...
  // GitHub handlers
  async function handle_github_unlink() {
    if (!confirm("Unlink this repository?")) return;
    try {
      await fetch(`/api/projects/${project_id}/github/link`, {
        method: "DELETE",
  ...
  async function handle_github_push() {
    const message = prompt("Commit message:", "Update from darkpearl");
    if (!message) return;
    try {
      const response = await fetch(`/api/projects/${project_id}/github/push`, {
        method: "POST",
        headers: {
  ...
  async function handle_github_pull() {
    if (!confirm("Pull from GitHub? A snapshot will be created first.")) return;
    try {
      const response = await fetch(`/api/projects/${project_id}/github/pull`, {
        method: "POST",
        headers: { Authorization: `Bearer ${pb.authStore.token}` },
      });
      const data = await response.json();
  ...
  function handle_github_linked(repo: { full_name: string }) {
    store.refresh();
  }
...
<svelte:head>
  <title>{project_title || "Loading..."} - darkpearl</title>
</svelte:head>
    ...
    {#key project_id}
      ...
      <div class="hidden md:block">
        <Header
          ...
          {current_tab}
          {active_tool_tabs}
          github_repo={store?.project?.settings?.github_repo}
          ...
          on_github_link={() => show_github_link_dialog = true}
          on_github_unlink={handle_github_unlink}
          on_github_push={handle_github_push}
          on_github_pull={handle_github_pull}
        />
      </div>
      ...
      <div class="hidden md:flex flex-1 overflow-hidden">
        {#if preview_position === "bottom"}
          ...
          <PaneGroup
            direction="vertical"
            class="flex-1"
            autoSaveId="darkpearl-builder-vertical"
          >
            <Pane
              bind:this={left_pane}
              defaultSize={50}
        ...
        {:else if preview_position === "left"}
          ...
          <PaneGroup
            ...
            class="flex flex-1"
            autoSaveId="darkpearl-builder-left"
          >
            <Pane
              bind:this={right_pane}
              defaultSize={50}
        ...
        {:else}
          ...
          <PaneGroup
            direction="horizontal"
            class="flex flex-1"
            autoSaveId="darkpearl-builder-desktop"
          >
            <Pane
              bind:this={left_pane}
              defaultSize={50}
      ...
      <div class="flex md:hidden flex-col flex-1 overflow-hidden">
        ...
        <PaneGroup
          direction="vertical"
          class="flex-1"
          autoSaveId="darkpearl-builder-mobile"
        >
          <!-- Preview (top pane) -->
          <Pane
      ...
      <!-- GitHub Link Dialog -->
      <GitHubLinkDialog
        bind:open={show_github_link_dialog}
        project_id={store?.project?.id ?? ""}
        onlinked={handle_github_linked}
      />
    {/key}
  </div>
{/if}
