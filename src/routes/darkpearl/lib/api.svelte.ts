// API client functions for darkpearl admin interface
...
export async function build_app(project_id: string): Promise<any> {
	...
	// Build darkpearl_modules for SSR (content/design/data inlined during server render)
...
function generate_production_html({ body, head = '', hydration_js, config }: ProductionHtmlOptions): string {
	...
	// Generate $tk module (darkpearl utilities)
	const darkpearl_module = generate_darkpearl_module(config.project_id || '')
	const darkpearl_url = `data:text/javascript,${encodeURIComponent(darkpearl_module)}`
 ...
 * This creates a db object with collection accessors that proxy to the /_dp/data API
...
if (typeof window !== 'undefined') {
  ...
  const sse = new EventSource('/_dp/realtime/' + PROJECT_ID)
...
/**
 * Generate the $tk module code (darkpearl utilities)
 */
function generate_darkpearl_module(project_id: string): string {
	return `
...
export function asset(filename, options) {
  ...
  let url = '/_dp/assets/' + PROJECT_ID + '/' + filename
  const params = []
