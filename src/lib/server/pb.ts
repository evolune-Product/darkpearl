...
export async function listProjects(): Promise<Project[]> {
	...
	const records = await pb.collection('_dp_projects').getList(1, 500)
...
export async function getProject(id: string): Promise<Project | null> {
	...
	try {
		return await pb.collection('_dp_projects').getOne(id)
	} catch (err: any) {
...
export async function getProjectByDomain(domain: string): Promise<Project | null> {
	...
	try {
		...
		const result = await pb.collection('_dp_projects').getFirstListItem(
			`domain = "${normalized}"`
		)
...
export async function createProject(data: {
	...
	return await pb.collection('_dp_projects').create(project_data)
}
...
export async function updateProject(projectId: string, data: any): Promise<Project | null> {
	...
	const task = prev.catch(() => { }).then(async () => {
		return await pb.collection('_dp_projects').update(projectId, data) as unknown as Project
	})
...
export async function deleteProject(projectId: string): Promise<boolean> {
	...
	await pb.collection('_dp_projects').delete(projectId)
	return true
}
...
export async function trackAvailableDomain(hostname: string): Promise<void> {
	...
	try {
		...
		let record: any
		try {
			record = await pb.collection('_dp_settings').getOne('available_domains')
		} catch {
			// Create if doesn't exist
			record = await pb.collection('_dp_settings').create({
		...
		await pb.collection('_dp_settings').update('available_domains', {
...
export async function getAvailableDomains(): Promise<AvailableDomain[]> {
	...
	try {
		const record = await pb.collection('_dp_settings').getOne('available_domains')
		const domains: AvailableDomain[] = record?.value?.domains || []
