...
async function rollup_worker({ component, head = { code: '', data: {} }, hydrated, buildStatic = true, css = 'external', format = 'esm', dev_mode = false, sourcemap = false, runtime = [], darkpearl_modules = {} }) {
	const final = {
		ssr: '',
		dom: '',
		error: ''
	...
	// Virtual modules for $content, $design, $data, $darkpearl
	// For SSR: inline the data. For client: leave as external (import map resolves them)
	const { content = {}, design = {}, data = {} } = darkpearl_modules
		...
		try {
			return await rollup({
				...
				external: (id) => {
					...
					if (id === '$content' || id === '$design' || id === '$data' || id === '$darkpearl') {
				...
				plugins: [
					commonjs,
					{
						...
						async resolveId(importee, importer) {
							...
							// 2) DarkPearl virtual modules ($content, $design, $data, $darkpearl)
							if (importee === '$content' || importee === '$design' || importee === '$data' || importee === '$darkpearl') {
								if (isSSR) {
