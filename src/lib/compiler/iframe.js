...
/**
 * Generate the $darkpearl module code (darkpearl utilities)
 * @param {string} project_id
 * @returns {string}
 */
const generate_darkpearl_module = (project_id) => {
  return `
...
export function asset(filename, options) {
  ...
  let url = PROJECT_ID ? '/_dp/assets/' + PROJECT_ID + '/' + filename : '/_dp/assets/' + filename
  const params = []
...
/**
 * Fetch external resources through the darkpearl proxy (bypasses CORS)
...
`.trim();
  ...
  const tk_module = generate_darkpearl_module(project_id);
