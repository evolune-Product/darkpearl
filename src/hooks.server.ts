...
export const handle: Handle = async ({ event, resolve }) => {
	...
	// Handle CORS preflight for /_dp/ routes (data API, assets, realtime)
	if (url.pathname.startsWith('/_dp/') && event.request.method === 'OPTIONS') {
	...
	if (!url.pathname.startsWith('/api/') && !url.pathname.startsWith('/_pb/') && !url.pathname.startsWith('/_dp/')) {
	...
	// Add CORS headers to /_dp/ responses
	if (url.pathname.startsWith('/_dp/')) {
