# darkpearl - Claude Code Documentation
...
darkpearl is an open-source, self-hosted AI development platform—think Lovable, but running on your own server alongside the apps it builds. It provides database, editor, and hosting all at `/darkpearl`, enabling you to ship production CRUD apps in an afternoon.
...
- **Two Pocketbase collections:**
  ...
  - `_dp_settings` - App configuration (LLM keys, etc.)
...
### `_dp_projects` Collection Schema
...
### `_dp_settings` Collection Schema
...
Each project is associated with a domain. Users can point multiple domains to a single darkpearl server:

```
calculator.myserver.com/                → Serves calculator production app
calculator.myserver.com/darkpearl/studio  → Edit calculator app
calculator.myserver.com/darkpearl/dashboard → See ALL apps
...
- `/darkpearl/studio` - Edit the project for current domain
- `/darkpearl/studio?id=X` - Edit specific project by ID
- `/darkpearl/dashboard` - List all projects
- `/darkpearl/new?domain=X` - Create new project for domain
...
```
┌───────────────────────────────────────────────────────────┐
│ darkpearl · My Project                  [Vibe] [Deploy]   │
├────────────────────────────┬──────────────────────────────┤
...
│   │   ├── setup/              # First-time setup wizard
│   │   │   └── +page.svelte
│   │   ├── darkpearl/          # Main admin interface
...
### ProjectStore (`src/routes/darkpearl/studio/project.svelte.ts`)
...
// Projects
const projects = await pb.collection("_dp_projects").getFullList();
const project = await pb.collection("_dp_projects").getOne(id);
await pb.collection("_dp_projects").update(id, { frontend_code: "..." });

// Real-time subscriptions
pb.collection("_dp_projects").subscribe(id, (e) => {
  if (e.action === "update") {
    // Handle update
  }
});
...
- Admin UI: http://localhost:5173/darkpearl
- Setup: http://localhost:5173/setup
- Settings: http://localhost:5173/darkpearl/settings
...
```env
# LLM Configuration (set via /darkpearl/settings UI, stored in _dp_settings)
...
- Admin UI (`/darkpearl/*`) requires login
...
- `src/routes/darkpearl/+layout.svelte` - Auth guard
...
- Visit `/darkpearl/settings` to verify LLM configuration
